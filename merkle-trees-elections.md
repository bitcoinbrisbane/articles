...
The whole thing is about 30 lines of Python using the standard library only. Build the leaves, fold them into a root, then check that any given voter's leaf is in it.
...
It would not stop someone yelling "rigged" on social media. And it wouldn't stop someone setting fire to a drop box on a Sunday afternoon, or posting a stale photo of recycled envelopes and captioning it "found in a dumpster." But it would change what happens _next_: instead of a registrar issuing a denial that lands as just another voice in the noise, every voter could check their own ballot against the published root and see for themselves. When Albo said "did she though?", Hanson could answer with an audit. When someone says an election was rigged, the registrar should be able to do the same, or you could too after a donation, in about ten seconds on your phone.
